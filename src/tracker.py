...
class data:
    ...
    def to_csv(self):
        ...
        if not os.path.exists('../data'):
            os.mkdir('../data')
            self.logger.log('tracker', LogLevel.INFO, 'Created data folder at ../data')
            
        df = pd.DataFrame([self.__dict__()])
        if not os.path.exists(DATA_LOCATION):
            df.to_csv(DATA_LOCATION, index=False)
            self.logger.log('tracker', LogLevel.INFO, 'Created data csv at {DATA_LOCATION}')
        else:
            ...
            self.logger.log('tracker', LogLevel.INFO, 'Wrote data to csv.')
